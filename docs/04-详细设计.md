...
**设计要点**：将所有状态可用动作收敛为枚举方法，杜绝业务代码中散落的 `if ("draft".equals(status))` 字符串比较，编译期即可约束，配合 `orders.status` ENUM 形成双重保障。

### 4.3.2 状态流转表

| 动作 | 接口 | 前置 | 后置 | 副作用 |
| ---- | ---- | ---- | ---- | ---- |
| 生成出货单 | `POST /api/orders/{id}/ship` | draft | pending | 插入 `shipments`（计划发货日、物流单号选填） |
| 确认发货 | `POST /api/orders/{id}/confirm-ship` | pending | shipped | 更新 `shipments.shipment_date` 为实际发货日、补物流单号 |
| 录入收款 | `POST /api/orders/{id}/pay` | shipped | shipped / paid | 插入 `payments`；累计 = 总额时转 paid |
| 取消订单 | `POST /api/orders/{id}/cancel` | draft / pending | cancelled | 无 |
...
SELECT
  CASE
    WHEN DATEDIFF(CURDATE(), s.shipment_date) BETWEEN 0  AND 30 THEN '0-30'
    WHEN DATEDIFF(CURDATE(), s.shipment_date) BETWEEN 31 AND 60 THEN '31-60'
    WHEN DATEDIFF(CURDATE(), s.shipment_date) BETWEEN 61 AND 90 THEN '61-90'
    ELSE '90+'
  END AS bucket,
  COALESCE(SUM(o.total_amount - COALESCE(t.paid_amount, 0)), 0) AS amount
FROM orders o
JOIN shipments s ON o.id = s.order_id
LEFT JOIN (SELECT order_id, SUM(amount) paid_amount FROM payments GROUP BY order_id) t
       ON o.id = t.order_id
WHERE o.status = 'shipped'
GROUP BY bucket;
```
...
| 方法 | 路径 | 说明 |
| ---- | ---- | ---- |
| GET | `/api/dashboard` | 仪表盘：三档金额、账龄分布、欠款 Top10 客户 |
